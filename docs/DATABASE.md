...
系統不再內建或自動建立範例工地，空資料庫從空白工地清單開始。本機預覽亦不預載範例工地及示範紀錄。此調整不刪除既有資料庫內容，也不再依範例名稱自動修改或刪除工地。

刪除工地僅限建立者（`created_by`；舊資料缺漏時採 `owner_id`）。受邀的 manager/editor/viewer 以及非建立者的系統管理員皆不能刪除工地；既有編輯、閱覽授權不變。刪除 API 與資料庫 DELETE 條件均檢查建立者身分。首頁依建立者及成員關係分組，管理員額外可見但未參與的工地另列「其他工地（系統管理）」。
...
- 自 `eztodo_26091401` 起，ICS 包含 `VALARM / ACTION:DISPLAY`。未完成的未來行程依設定的台灣時間提醒；全天／跨日行程在開始日 09:00（台灣時間）提醒一次。已完成／取消及已過提醒時間的項目不新增提醒。UTC 絕對觸發時間避免全天行程在午夜響鈴。
- 既有訂閱保留相同 UID 及 token；feed revision 將 LAST-MODIFIED 至少提升到此功能發布時間，供 Apple 重新整理辨識更新。Apple 端仍須允許通知、取消「移除提示」；網站無法強制開啟裝置通知，也不保證短時間內新建的行程能及時抓取及提醒。
- Mac 訂閱時選 iCloud 可跨同帳號裝置使用；每個訂閱顏色可在 Apple 自行調整。參考 [Apple 訂閱行事曆說明](https://support.apple.com/zh-tw/guide/calendar/icl1022/mac)。

### 原始整合順序

常用設定介面自 `eztodo_26091401` 分離至 `src/CommonSettings.jsx`：支援搜尋別名／單位／規格、啟用狀態篩選、視窗編輯、連續新增、材料單位快捷選擇。排序僅在無搜尋且顯示全部時可操作，依項目 ID 排序；永久刪除保留明確警告，新增／編輯失敗不清空輸入。資料仍經原有 `persistCommonSettings` API 保存，不變更 ID、日報關聯或統計架構。
