...
- 原「測試分組1／2／3」直接成為公司群組（可改名），其他既有單位標籤也合併到相同名稱群組。原帳號自動歸入原公司，保留帳號開關與工地授權；原可編輯帳號採可保留原管理能力的階級，原僅閱覽帳號維持僅閱覽。已經手動指定群組的帳號不重分派。
- 一次性 `legacy-company-groups-v1` 遷移使用交易、鎖及完成標記；失敗整批回復，下次啟動可重試。公司改名或帳號移出群組後，不會被下次啟動改回測試名稱或重新分組。
- 註冊、管理員新增帳號、帳號列表及篩選統一讀取 `account_groups`，不再另存一套任意公司標籤。註冊只提供公司 ID／名稱選項，不公開人員或階級資料，新註冊帳號固定採所選公司的最低階級（不能由請求指定更高階級）。管理員新增帳號可同時指定階級。
- `organization_name` 僅保留相容性；已分組帳號顯示名稱以群組資料為準。移出群組時清除舊名稱，恢復帳號原有開關，操作前有確認提示。
